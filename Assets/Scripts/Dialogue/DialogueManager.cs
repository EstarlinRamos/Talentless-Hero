...
public class DialogueManager : MonoBehaviour
{
    ...
    /// <summary>
    /// Time.time when the last dialogue ended. Used by PlayerAttack to prevent
    /// sword swings immediately after closing dialogue with Space.
    /// </summary>
    public static float LastDialogueEndTime { get; private set; } = -10f;
    ...
    [Tooltip("Sound played when the player selects a dialogue choice.")]
    [SerializeField] private AudioClip choiceClickSound;

    [Tooltip("Volume for the choice click sound (0-1).")]
    [Range(0f, 1f)]
    [SerializeField] private float choiceClickVolume = 0.6f;
    ...
    [Header("Dialogue SFX")]
    [Tooltip("Sound played when dialogue first opens.")]
    [SerializeField] private AudioClip dialogueOpenSFX;

    [Tooltip("Sound played when advancing to the next line (Space/Enter/Click).")]
    [SerializeField] private AudioClip dialogueAdvanceSFX;

    [Range(0f, 1f)]
    [SerializeField] private float dialogueSFXVolume = 0.4f;
    ...
    public event Action OnKickRequested;
    ...
    public void StartDialogue(DialogueData dialogue)
    {
        ...
        PlayDialogueSFX(dialogueOpenSFX);
    ...
    private void Update()
    {
        ...
        {
            advancePressed = Keyboard.current.spaceKey.wasPressedThisFrame ||
        ...
        {
            _skipRequested = true;
            PlayDialogueSFX(dialogueAdvanceSFX);
        }
    ...
    private void ShowChoices(List<DialogueChoice> choices)
    {
        ...
        for (int i = 0; i < choiceButtons.Count; i++)
        {
            if (i < choices.Count)
            {
                ...
                {
                    // Play click SFX directly since RemoveAllListeners
                    // wipes any externally registered listeners
                    if (choiceClickSound != null && AudioManager.Instance != null)
                        AudioManager.Instance.PlaySFX(choiceClickSound, choiceClickVolume);
    ...
    private void PlayDialogueSFX(AudioClip clip)
    {
        if (clip == null) return;
        if (AudioManager.Instance != null)
            AudioManager.Instance.PlaySFX(clip, dialogueSFXVolume);
    }
